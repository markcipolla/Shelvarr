 ...
 * Author data layer
 *
 * DB- and OpenLibrary-backed author operations shared by the web server
 * actions and the background queue handlers. Lives in the services package so
 * both consumers import it from one place (previously the queue handlers
 * reached into the web app via the `@/` alias, which does not resolve outside
 * the Next.js build).
 */
