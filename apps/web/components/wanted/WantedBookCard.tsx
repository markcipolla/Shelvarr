...
export function WantedBookCard({ book, onFindDownloads }: WantedBookCardProps) {
  ...
  return (
    <div className="book-cover-trigger group">
      <BookCover
        src={book.cover_url}
        title={book.title}
        author={book.author}
        overlay={
          ...
          <div className="absolute inset-0 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity flex items-center justify-center gap-2">
            <button
              onClick={() => onFindDownloads?.(book)}
              className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded text-sm font-medium transition-colors shadow-lg"
              disabled={loading}
            >
              Find Downloads
            </button>
            <div className="relative">
              <button
                ...
                className="bg-shelvarr-surface hover:bg-shelvarr-border text-white p-1.5 rounded transition-colors shadow-lg"
              >
                <MoreIcon />
              </button>
              {menuOpen && (
